<?php 
        ...
        if(isset($_POST['create_store'])){
            ...
            if(!$error){
                ...
                if($stmt->execute()){
                    ...
                    $token = sha1($last_id);
                    $url = "http//".$_SERVER['SERVER_NAME'].'/bentiZone/home/verifyStore.php?id='.base64_encode($last_id).'&token='.$token;
    ...
    function newArrivals(){
        require "./includes/dbconfig.php";
        // Getting page number and constants
        ...
        // querry total pro
        $count = $con->prepare("SELECT count(*) from products");
        $count->execute();
        $num = $count->fetchColumn();
        $totalPages = ceil($num / $ppp);
        $secondLast = $totalPages - 1;
        // querrying from db
        $stmt = $con->prepare("SELECT * FROM products LIMIT $offset ,$ppp");
        $stmt->setFetchMode(PDO:: FETCH_ASSOC);
        $stmt->execute();
        while($row = $stmt->fetch()):
            echo
        ...
        echo "<div class = 'justify-content-center pull-right'><ul class='pagination justify-content-center mt-3'><li class='page-item ";if($pageNo <= 1) echo "disabled"; echo "'><a href='?roll=".$prev."' class = 'page-link rounded-pill'>Previous</a></li>";
                if($totalPages <= 10){
                    for($i = 1; $i <= $totalPages; $i++){
                        // if($i == $pageNo){
                        //     echo "<li class='page-item'><a href='?roll=".$i."' class = 'page-link rounded-pill act'>".$i."</a></li>";
                        // }else{
                        //     echo "<li class='page-item'><a href='?roll=".$i."' class = 'page-link rounded-pill'>".$i."</a></li>";
                        // }
                        echo "<li class='page-item "; if($i == $pageNo) echo "active"; echo "'><a href='?roll=".$i."' class = 'page-link rounded-pill'>".$i."</a></li>";
                    }
                }elseif($totalPages > 10){
                    
                }
        echo " 
        <li class='page-item ";if($pageNo == $totalPages) echo "disabled"; echo "'><a href='?roll=".$next."' class = 'page-link rounded-pill'>Next</a></li>";
        if($pageNo < $totalPages){
            echo "<li class='page-item'><a href='?roll=".$totalPages."' class = 'page-link rounded-pill'>Last</a></li>";
        }
        echo "</ul></div>";
    }
    function hotTrends(){
            ...
            while($row = $stmt->fetch()):
                ...
                while($row = $stmt0->fetch()):
                    ...
                    <tr>
                        ...
                        <td class='border-0 align-middle'><strong><input type='number' class='form-control col-md-3 text-success' name='".$cart_id."' value = '1'></strong></td>
    ...
    function pagination(){
        ...
        // Getting page number and constants
        if(isset($_GET['roll']) && $_GET['roll'] != 0){
            $pageNo = base64_decode($_GET['roll']);
        }else{
            $pageNo = 1;
        }
        $ppp = 3;
